...
#This file has helper librarys used by facebook_api.py
#InstaFeed should never call any of these methods directly.
#InstaFeed should use the methods defined in facebook_api.py to communicate with Facebook.

#https://gist.github.com/1196739
import os.path
import os
import stat
import json
...
class _RequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        TOKEN_EXPIRES = params.get('expires_in', [None])[0]

        if ACCESS_TOKEN:
            data = {'scope': AUTH_SCOPE,
                    'access_token': ACCESS_TOKEN}
            open(LOCAL_FILE,'w').write(json.dumps(data))
            self.wfile.write("You have successfully logged in to facebook with InstaFeed! "
